...
export default function DealRoom() {
  ...
  const { user } = useAuth(); // user.role = "buyer" or "seller"

  if (!matches || matches.length === 0) {
    return (
      <div className="p-6">
        <h2 className="text-xl font-semibold">Deal Room</h2>
        <p className="mt-2 text-slate-600">No active deals yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
      {matches.map((match) => {
        ...
        return (
          <div key={match.matchId} className="space-y-6">
            ...
            {user.role === "buyer" && seller && (
              <SellerProfile seller={seller} onClose={() => {}} />
            )}
